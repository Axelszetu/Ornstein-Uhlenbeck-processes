...
```
## Brownian motion
We use the following function to simulate brownian motion and plot sample trajectories:
```{r}
SimBM <- function(d = 2, t = 10, N = 1000){
  #generating samples of the appropriate scale
  samples = rnorm(n = d*N, mean = 0, sd = sqrt(t/N))
  #making a 0-start
  samples = c(numeric(d),samples)
  #construction of path
  increment_matrix <- matrix(data = samples, nrow = N+1, ncol = d, byrow = T)
  #we now wish to apply cumsum columnwise
  path_matrix <- apply(increment_matrix, 2, cumsum)
  #Return matrix with each dimension in column
  path_matrix
}
```
```{r}
BM_paths <- SimBM(d = 6, t = 10, N = 10000)
plot_data <- as.data.frame(BM_paths)
plot_data$t <- seq(from = 0, to = 10, by = 10/10000)
melded_plot_data <- melt(plot_data, id.vars = "t")
ggplot(melded_plot_data, aes( x=t, y=value, colour=variable, group=variable )) + 
  geom_line() + theme_minimal()
```
## Scalar Ornstein-Uhlenbeck processes
We use the following to simulate scalar OU-processes.
```{r}
SimScalarOU <- function(W_0 = 0, t = 10, N = 10000, theta = 0.1, sigma = 1){
...
scalar_ou <- SimScalarOU()
```
Show a path
```{r}
plot_data <- as.data.frame(scalar_ou)
plot_data$t <- seq(from = 0, to = 10, by = 10/10000)
melded_plot_data <- melt(plot_data, id.vars = "t")
ggplot(melded_plot_data, aes( x=t, y=value, colour=variable, group=variable )) + 
  geom_line() + theme_minimal()
```
## MLE of scalar Ornstein-Uhlenbeck process
We simulate 1000 paths from an OU process with theta = 0.1.
For each we compute the MLE and plot the results for an empirical distribution of the MLE.
```{r}
n <- 1000
theta <- 0.1
t <- 10
...
ggplot(data = melt(simulated_values), aes(x = value)) + geom_histogram() + geom_vline(aes(xintercept = 0.1, color = "red"))
```
...
A_D_CV <- OU_Dantzig(X = process, dt = dt, lambda = lambda_D)
```
```{r}
A_D_CV_melt <- melt(t(A_D_CV), varnames = c("Column", "Row"))
ggplot(data = A_D_CV_melt, mapping = aes(x = Column, y = Row, fill = value)) + geom_tile(color = "white") +
  scale_fill_gradient2(low = rgb(0,0,219,maxColorValue = 255), high = rgb(219,0,0,maxColorValue = 255), mid = "white",
                       midpoint = 0, limit = limit, space = "Lab", 
                       name="Drift") +
  scale_y_reverse() + coord_fixed() + ggtitle("A_D_CV")
```


# Results

We compute norm of the error matrices and compare them.
```{r}
normalization <- sum(abs(A))
error_mle <- sum(abs(A-A_hat))/normalization
error_L <- sum(abs(A-A_L_CV))/normalization
error_D <- sum(abs(A-A_D_CV))/normalization
errors <- c(error_mle, error_L, error_D)
```

Manual calibration of estimators
```{r}
lambda_L_manual <- 0.1
A_L_manual <- OU_Lasso(process, dt = dt, lambda = lambda_L_manual)
lambda_D_manual <- 0.01
A_D_manual <- OU_Dantzig(process, dt = dt, lambda = lambda_D_manual)
```
Computation and comparison of relative errors:
```{r}
error_L_manual <- sum(abs(A-A_L_manual))/normalization
error_D_manual <- sum(abs(A-A_D_manual))/normalization
errors <- c(error_mle, error_L_manual, error_D_manual)
```

Plots for manually calibrated estimators
```{r}
A_L_manual_melt <- melt(t(A_L_manual), varnames = c("Column", "Row"))
ggplot(data = A_L_manual_melt, mapping = aes(x = Column, y = Row, fill = value)) + geom_tile(color = "white") +
  scale_fill_gradient2(low = rgb(0,0,219,maxColorValue = 255), high = rgb(219,0,0,maxColorValue = 255), mid = "white",
                       midpoint = 0, limit = limit, space = "Lab", 
                       name="Drift") +
  scale_y_reverse() + coord_fixed() + ggtitle("A_L_manual")

A_D_manual_melt <- melt(t(A_D_manual), varnames = c("Column", "Row"))
ggplot(data = A_D_manual_melt, mapping = aes(x = Column, y = Row, fill = value)) + geom_tile(color = "white") +
  scale_fill_gradient2(low = rgb(0,0,219,maxColorValue = 255), high = rgb(219,0,0,maxColorValue = 255), mid = "white",
                       midpoint = 0, limit = limit, space = "Lab", 
                       name="Drift") +
  scale_y_reverse() + coord_fixed() + ggtitle("A_D_manual")
```
